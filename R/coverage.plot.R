coverage.plot <-
function(coverageAll, targets, chr, Start, End, Offset=0,
                          col.line=1, col.target="orange", col.offset="yellow", xlab, ylab, ...){
  ...
  ma <- max(covsel)
  mi <- .04 * ma
  plot(Start:End, covsel, ylim=c(-mi, ma), type="l", col=col.line, xlab=xlab, ylab=ylab, ...)
  abline(h=0, lty=3)
