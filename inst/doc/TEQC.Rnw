...
This file format is very general and hence
...
identifier of the read pair. Our example data was derived by paired-end sequencing,
...
the first one containing the original positions of the single reads without partners, and the other one containing the
merged pairs positions.
...
a first impression on that (see figure \ref{chrom.barplot}). The plot is more useful when there
are targets only on some of the chromosomes, for which hence a high number of reads is expected.\\

\noindent
\Rcode{> par(mar=c(8,4,4,2))}\\
\Rcode{> chrom.barplot(reads)}\\
...
@
\vspace{-0.4cm}
\caption{{\small \label{chrom.barplot} Boxplots of aligned reads per chromosome.}}
\end{center}
\end{figure}
...
and/or located in a target region. It returns the average coverage over all targeted bases.
When option \Rfunarg{perBase} is set to \Rfunarg{TRUE}, a list is returned that besides
the average coverage (list element \Robject{avgTargetCoverage}) has the two elements
...
has the additional element \Robject{targetCoverages}, that is the input \Robject{targets}
\Rclass{RangedData} table, now including average coverages per target region as a 'values' column.
...
Also the coverage uniformity plot shown above (figure \ref{coverage.uniformity}) can be
produced for several samples within the same graph. The function can be called repeatedly,
while specifying option \Rfunarg{add=TRUE} (see figure \ref{cov.uniformity.2}).
